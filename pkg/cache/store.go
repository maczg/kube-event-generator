...
func NewStore(clientset *kubernetes.Clientset, logLevel logger.LogLevel) *Store {
	ni := &Store{
		mu:        &sync.RWMutex{},
		clientset: clientset,
		log:       logger.NewLogger(logLevel, "node-informer"),
...
func (s *Store) onAddNode(obj interface{}) {
	...
	s.log.Debug("[onAdd] node %s added", node.Name)
}
...
func (s *Store) addPod(obj interface{}) {
	...
	if pod.Status.Phase == v1.PodPending {
		s.log.Debug("[onAdd] pod %s added to pending queue", pod.Name)
		s.stats.UpdatePendingQ(pod, AddPodToPendingQ)
	}
	if nodeName := pod.Spec.NodeName; nodeName != "" {
		s.log.Debug("[onAdd] pod %s added to node %s", pod.Name, nodeName)
...
func (s *Store) updatePod(oldObj, newObj interface{}) {
	...
	if newPod.Status.Phase == v1.PodPending {
		if _, ok := s.stats.PendingQ[NewKey(newPod)]; !ok {
			s.log.Debug("[onUpdate] pod %s added to pending queue", newPod.Name)
	...
	if oldPodNodeName != newPodNodeName {
		...
		if oldPodNodeName == "" && newPodNodeName != "" {
			s.log.Debug("[onUpdate] %s is now on %s, removing from pending queue", oldPod.Name, newPodNodeName)
		...
		if newPodNodeName == "" {
			s.log.Warn("[onUpdate] pod %s has newNodePodName empty. Deleted?", newPod.Name)
		}
	}
}
...
func (s *Store) deletePod(obj interface{}) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	s.log.Debug("[onDelete] pod %s deleted", pod.Name)

	key := NewKey(pod)

	switch pod.Status.Phase {
	case v1.PodRunning:
		s.log.Debug("[onDelete] running pod %s delete", pod.Name)
		if nodeName := pod.Spec.NodeName; nodeName != "" {
	...
	case v1.PodPending:
		...
		s.log.Debug("[onDelete] pending pod %s delete", pod.Name)
...
func (s *Store) WatchEvery(seconds int) {
	s.log.Info("logging every %d seconds", seconds)
	...
	for {
		...
		case <-ticker.C:
			...
			s.log.Info(statusbuilder.String())
			s.mu.RUnlock()
		case <-s.stopCh:
			return
		}
