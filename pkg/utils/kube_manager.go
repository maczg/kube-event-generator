...
func (m *KubernetesManager) DescribeCluster() {
	...
	for _, n := range items {
		var resType []string
		var qty []string
		for resourceType, quantity := range n.Status.Allocatable {
			resType = append(resType, resourceType.String())
			qty = append(qty, quantity.String())
		}
		logrus.Infof("Node: %s resources: %v with quantity: %v", n.Name, resType, qty)
	}
}
